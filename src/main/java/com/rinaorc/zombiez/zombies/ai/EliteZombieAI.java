...
public class EliteZombieAI extends ZombieAI {
    ...
    public EliteZombieAI(ZombieZPlugin plugin, LivingEntity entity, ZombieType zombieType, int level) {
    ...
    private void divineBolt(Player target) {
        ...
        for (int i = 0; i < 3; i++) {
            final int bolt = i;
            plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
                if (!zombie.isValid() || !target.isOnline() || target.isDead())
                    return;
    ...
    private void judgement(Player target) {
        ...
        // Exécution
        plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
            if (!zombie.isValid() || !target.isOnline() || target.isDead() || hasRespawnProtection(target))
                return;
    ...
    private void energyBurst(Player target) {
        ...
        plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
            ...
            if (projectileLoc.distance(zombie.getLocation()) > 40)
                task.cancel();
        }, 0L, 1L);
    }
    ...
    public void onAttack(Player target) {
        ...
        // CRITIQUE: Ne pas faire de dégâts si le joueur est déjà mort (empêche boucle
        // infinie)
        if (target.isDead())
            return;
        ...
        // Effet spécial selon le type (seulement si vivant)
        if (!target.isDead()) {
            switch (zombieType) {
